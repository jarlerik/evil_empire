#!/bin/bash
# App-only deploy: rebuild web assets, sync to existing S3 bucket, invalidate CDN.
# Skips `sam build`/`sam deploy` so it's much faster than ./deploy.sh when only
# the web app changed (the common case).
# Usage: ./deploy-app.sh [staging|prod]
set -e
...
echo ""
echo "App deployment ($STAGE) complete!"
