...
- The Supabase project has email confirmation enabled. Sign-up via API succeeds but sign-in returns `email_not_confirmed` until the user confirms via email link. This blocks full end-to-end authenticated testing from the cloud VM unless the project owner disables email confirmation or provides pre-confirmed test credentials.
- React Native web `TextInput` components don't reliably accept input from browser automation tools (e.g., Playwright, CDP). Setting DOM `.value` doesn't update React state. For GUI testing of forms, use the browser console to call Supabase or app methods directly instead of trying to type into fields.
